...
faturayı Hype Vision keser. Ödeme sağlayıcıları, sözleşmelerdeki
ünvan/vergi/adres bilgilerinin POS başvurusundakiyle **birebir** aynı olmasını
şart koşar.
...
| 6 | Kart şeması rozetleri | ⚠️ Şu an yazıyla (Visa · Mastercard · Troy) — sağlayıcı logo paketi bekleniyor |
| 7 | Sağlayıcı logosu | ⚠️ `PAYMENT_PROVIDER.logo` boş; PayTR logo paketi gelince doldurulacak |

Ayrıca **Ön Bilgilendirme Formu** (`/yasal/on-bilgilendirme-formu`) da eklendi —
mesafeli satışta yasal olarak zorunludur.
...
## Ödeme entegrasyonu — PayTR iFrame API

`/kayit` sayfası üç adımlı: **seçim → bilgiler → ödeme**. Ödeme formu PayTR'ın
iframe'i içinde açılır; kart bilgileri bizim sunucumuza hiç ulaşmaz.

| Uç | İşi |
|---|---|
| `POST /api/paytr/token` | Tutarı **sunucuda** hesaplar, iframe_token alır (1. ADIM) |
| `POST /api/paytr/bildirim` | PayTR'ın ödeme sonucunu bildirdiği adres (2. ADIM) |
| `GET /api/paytr/durum` | Sonuç sayfasının siparişi yokladığı uç |

Kod: `netlify/functions/` ve `netlify/lib/`.
Kurulum, güvenlik kararları ve sorun giderme:
[`docs/ODEME-ENTEGRASYONU.md`](docs/ODEME-ENTEGRASYONU.md)

**Açmak için** (Netlify → Environment variables):
`PAYTR_MERCHANT_ID`, `PAYTR_MERCHANT_KEY`, `PAYTR_MERCHANT_SALT`,
`PAYTR_TEST_MODE=1`, `VITE_PAYMENT_ONLINE=true`

Anahtarlar tanımlı değilken kayıt akışı WhatsApp üzerinden yürümeye devam eder.
